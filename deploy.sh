...
echo "=== 1. Deploy GCP infrastructure with OpenTofu ==="
cd infra
tofu init
...
echo "=== 2. Get outputs ==="
STATIC_IP=$(tofu -chdir=infra output -raw ingress_static_ip)
IMAGE_REPO_FULL=$(tofu -chdir=infra output -raw artifact_registry_repo)
SSIP_DOMAIN=$(tofu -chdir=infra output -raw sslip_domain)
DB_PASSWORD=$(tofu -chdir=infra output -raw postgres_password)
...
# secret: DB password from tofu + API keys from env vars
sed \
...
sleep 10
kubectl -n what2cook exec deploy/app -- php vendor/bin/phinx migrate -e development
